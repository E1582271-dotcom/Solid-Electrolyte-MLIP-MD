...
## 结果（W6 baseline，来自 `data/metrics.json` + `metrics_long.json`）
对标实验：Li₆PS₅Cl 室温 ~3.15 mS/cm（烧结）/ 1.33 mS/cm（机械化学）。D@1000K 为 kinisi 估计（带误差棒）。

| MLIP（时长） | D@1000K (cm²/s) | σ(300K) 外推 (mS/cm) | Eₐ (eV) | 与实验 3.15 倍率 | Arrhenius R² | 备注 |
|---|---|---|---|---|---|---|
| MACE-MP-0 (50 ps) | 5.0×10⁻⁵ | 29.6 | 0.198 | 9.4× | 0.81 | 未微调；低温 MSD 未收敛 |
| MACE-MP-0 (150 ps) | 4.6×10⁻⁵ | 12.9 | 0.232 | 4.1× | 0.999 | 未微调；已收敛，Arrhenius 成直线 |
| MatterSim (50 ps) | 4.3×10⁻⁵ | 16.0 | 0.211 | 5.1× | 0.85 | 未微调；薄数据 |

**读出**：两个未微调通用势都系统性过预测单晶 σ（无晶界的上界），MatterSim 比 MACE 轻一档；把 MACE 从 50→150 ps，600 K 点归位、R² 0.81→0.999、过预测 9.4×→4.1×——证实低温摆动是 MSD 采样噪声而非真·非阿伦尼乌斯。微调（W7）目标是把 σ 进一步拉向实验。
