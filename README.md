...
**为什么先验会失手**：项目三生成的候选全部落进 P1 模型的"P1 对称性 + Family=unknown"盲区（见项目一 [figures/07_family_coverage.png](../project1_screening/figures/07_family_coverage.png)——78.3% 的真实候选都是这个状态），模型此时只能看组分统计量,分不出 Li₃PS₄ 和 LiPS₃ 谁更导电——两者先验分数只差 0.3 个量级,而真实电导率差了整整 5 个量级。**这正是漏斗需要第二层（MD）的理由**：粗先验负责把搜索空间从几十万压缩到几个,但排序内部的分辨率不够精细,必须靠更贵的方法接力。
