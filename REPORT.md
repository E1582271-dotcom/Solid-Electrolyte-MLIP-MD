...
微调势验证力 RMSE：基座 234 → 微调 **62.4 meV/Å**（3.75× 改善），验证能 RMSE 3.3 meV/atom。
...
- **未微调基座过预测**：两个通用势都系统性高估单晶 σ（MatterSim 比 MACE 轻一档）。它们在 MP 的 PBE 平衡态数据上训练，学到的是**近平衡的能量-力面**；对 Li⁺ 迁移过渡态与硫化物软骨架的细节外推不足，倾向低估势垒 → 高估 σ。**W8 生产**（更大胞+更长时间）把 σ 收到 **1.8× 实验、R²=0.999**，是纯 MLIP-MD 能达到的文献级一致。
