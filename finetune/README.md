...
> ⚠️ **Γ-only caveat (read before trusting the fine-tuned σ)**: Γ-point sampling under-samples
...
> model — Eₐ 0.265→0.331 eV, σ(300 K) 5.57→0.497 mS/cm (0.16× expt). Fine-tuning is only as
...
| 5. Re-run MD with fine-tuned model → re-analyse σ | `02_baseline_md.py --mace-model <path>` + `03` | ~1–2 h A40 | ✅ done — σ(300 K)=0.497 mS/cm, Eₐ=0.331 eV (REPORT.md §3) |
