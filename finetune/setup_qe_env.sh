#!/bin/bash
# W7 step 2 prep — install ase into an isolated PYTHONUSERBASE on the LOGIN node, so the
# labelling driver (11_label_qe.py, pure-python + ase) runs alongside the cluster QE module.
# Mirrors setup_mattersim.sh. Run on the login node (has network); label_qe.pbs reuses ~/asepkg.
set -e
